...
class ModelToolExport extends Model {
	...
	function uploadGoods( &$reader, &$database, $delete) 
	{
		$data = $reader->getSheet(0);
		$options = array();
		$i = 0;
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$kod_zakaza = $this->getCell($data,$i,$j++);
			$artikul = $this->getCell($data,$i,$j++);
			$descr = $this->getCell($data,$i,$j++);
			$descr = htmlentities( $descr, ENT_QUOTES, $this->detect_encoding($descr) );
			$min_zakaz = $this->getCell($data,$i,$j++);
			$ed_izmer = $this->getCell($data,$i,$j++);
			$cena = $this->getCell($data,$i,$j++);
			$stoim = $this->getCell($data,$i,$j++);
			$gruppa_cen = $this->getCell($data,$i,$j++);
			
			$options[$i] = array();
			$options[$i]['kod_zakaza'] = $kod_zakaza;
			$options[$i]['artikul'] = $artikul;
			$options[$i]['descr'] = $descr;
			$options[$i]['min_zakaz'] = $min_zakaz;
			$options[$i]['ed_izmer'] = $ed_izmer;
			$options[$i]['cena'] = $cena;
			$options[$i]['stoim'] = $stoim;
			$options[$i]['gruppa_cen'] = $gruppa_cen;
		}
		return $this->storeExcelIntoDatabase( $database, $options, $delete);
	}
	
	function storeExcelIntoDatabase( &$database, &$options, $delete)
	{
		if($delete){
			$sql = "START TRANSACTION;\n";
		
			$sql .= "TRUNCATE TABLE `".DB_PREFIX."product_excel`;\n";
			$this->multiquery( $database, $sql );
		}	
		...
		// store product options into the database
		$first = TRUE;
		$sql = "INSERT INTO `".DB_PREFIX."product_excel` (`kod_zakaza`,`artikul`,`material_description`, `min_zakaz`, `ed_izmer`, `cena`, `stoimost`, `gruppa_cen`) VALUES "; 
		foreach ($options as $reward) {
			$kod_zakaza = $reward['kod_zakaza'];
			$artikul = $reward['artikul'];
			$material_description = addslashes($reward['descr']);
			$min_zakaz = $reward['min_zakaz'];
			$ed_izmer = $reward['ed_izmer'];
			$cena = htmlspecialchars($reward['cena'], ENT_QUOTES);
			$cena = preg_replace('/,/i','.',$cena);
			$stoim = $reward['stoim'];
			$stoim = preg_replace('/,/i','.',$stoim);
			$gruppa_cen = htmlspecialchars($reward['gruppa_cen'], ENT_QUOTES);
			
			$first = FALSE;
			$sql .= "('$kod_zakaza','$artikul','$material_description', '$min_zakaz', '$ed_izmer', '$cena', '$stoim', '$gruppa_cen')";
			$sql .= ($first) ? "\n" : ",\n";
		}
	...
	protected function expectedGoodsHeading() {
		return array( "Код заказа", "Артикул/тип", "Описание материала в каталоге", "Мин.заказ", "Ед. изм.", "Цена в RUEUR без НДС", "Стоимость в RUEUR без НДС", "Группа цен материалов" );
	}


	function validateGoods( &$reader )
	{
		$expectedGoodsHeading = $this->expectedGoodsHeading();
		$data =& $reader->getSheet(0);
		return $this->validateHeading( $data, $expectedGoodsHeading );
	}
	...
	protected function expectedExcelSheets() {
		return array('Лист1');
	}
	...
	function validateExcelUpload( &$reader )
	{
		$expectedWorksheets = $this->expectedExcelSheets();
		$sheetNames = $reader->getSheetNames();
		foreach ($expectedWorksheets as $key=>$expectedWorksheet) {
			if (!isset($sheetNames[$key]) || ($sheetNames[$key]!=$expectedWorksheet)) {
				error_log(date('Y-m-d H:i:s - ', time()).$this->language->get( 'error_sheet_count' )."\n",3,DIR_LOGS."error.txt");
	...
	function upload( $filename, $excel, $delete=FALSE ) {
		...
		try {
			...
			if($excel==FALSE){
				...
				$ok = $this->validateExcelUpload( $reader );
				if (!$ok) {
					return FALSE;
				}
				$this->clearCache();
				$this->session->data['export_nochange'] = 0;
				$ok = $this->uploadGoods( $reader, $database, $delete);

...
class ControllerToolExport extends Controller { 
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			if ((isset( $this->request->files['upload'] )) && (is_uploaded_file($this->request->files['upload']['tmp_name']))) {
				...
				if(isset($this->request->post['goods'])){
					$code = $this->request->post['goods'];
				}else{
					$code = FALSE;
				}
				if(isset($this->request->post['del'])){
					$d = $this->request->post['del'];
				}else{
					$d = FALSE;
				}
				if ($this->model_tool_export->upload($file, $code, $d)===TRUE) {
		...
		$this->data['del'] = $this->language->get('del');
		$this->data['entry_description2'] = $this->language->get('entry_description2');
		$this->data['button_import'] = $this->language->get('button_import');
		$this->data['button_import_exel'] = $this->language->get('button_import_exel');
		...
		$this->data['action2'] = $this->url->link('tool/exportexel', 'token=' . $this->session->data['token'], 'SSL');
